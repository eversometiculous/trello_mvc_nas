...
def authorise_as_admin(fn):
    ...
    def wrapper(*args, **kwargs):
        ...
        if user.is_admin:
            ...
            return {'error': 'Not authorised to perform delete on cards. You need to be an adminstrator.'}, 403
    return wrapper
...
#    if not is_admin:
#        return {'error': 'Not authorised to delete cards. You need to be an adminstrator.'}, 403
    stmt = db.select(Card).filter_by(id=id)
    card = db.session.scalar(stmt)
    if card:
...
def update_one_card(id):
    ...
    if card:
        if str(card.user_id) != get_jwt_identity():
            return {'error': 'Only the owner of the card can edit the card'}, 403
